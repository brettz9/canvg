...
const canvg = function (target, s, opts) {
  ...
  if (isNullish(target) && isNullish(s) && isNullish(opts)) {
    const svgTags = doc.querySelectorAll('svg');
    for (let i = 0; i < svgTags.length; i++) {
      ...
      if (typeof svgTag.clientWidth !== 'undefined' && typeof svgTag.clientHeight !== 'undefined') {
        ...
        const rect = svgTag.getBoundingClientRect();
        c.width = rect.width;
...
const idRegex = /(#[^\s+>~.[:]+)/gu;
const classRegex = /(\.[^\s+>~.[:]+)/gu;
const pseudoElementRegex = /(::[^\s+>~.[:]+|:first-line|:first-letter|:before|:after)/gui;
const pseudoClassWithBracketsRegex = /(:[\w-]+\([^)]*\))/gui;
const pseudoClassRegex = /(:[^\s+>~.[:]+)/gu;
const elementRegex = /([^\s+>~.[:]+)/gu;
...
function getSelectorSpecificity (selector) {
  ...
  selector = selector.replace(/:not\(([^)]*)\)/gu, '     $1 ');
  selector = selector.replace(/\{[\s\S]*/gum, ' ');
...
function build (opts) {
  ...
  // trim
  svg.trim = function (s) { return s.replace(/^\s+|\s+$/gu, ''); };

  // compress non-ideographic spaces
  svg.compressSpaces = function (s) { return s.replace(/(?!\u3000)\s+/gum, ' '); };
  ...
  svg.Font = new function () {
    ...
    this.CreateFont = function (fontStyle, fontVariant, fontWeight, fontSize, fontFamily, inherit) {
      ...
      if (nodeEnv && fontFamily) {
        const trimmedFontParts = fontFamily.trim().split(',');
        for (let i = 0; i < trimmedFontParts.length; i++) {
  ...
  svg.ToNumberArray = function (s) {
    const a = (s || '').match(/-?(\d+(\.\d+)?|\.\d+)(?=\D|$)/gum) || []; // eslint-disable-line unicorn/no-unsafe-regex
  ...
  svg.Transform = function (v) {
    ...
    const data = svg.trim(svg.compressSpaces(v)).replace(/\)([a-zA-Z])/gu, ') $1').replace(/\)(\s?,\s?)/gu, ') ').split(/\s(?=[a-z])/u);
  ...
  svg.AspectRatio = function (ctx, aspectRatio, width, desiredWidth, height, desiredHeight, minX, minY, refX, refY) {
    ...
    aspectRatio = aspectRatio.replace(/^defer\s/u, ''); // ignore defer
  ...
  svg.Element.ElementBase = function (node) {
    ...
    // Microsoft Edge fix
    const allUppercase = new RegExp('^[A-Z-]+$', 'u');
  ...
  svg.Element.RenderedElementBase = function (node) {
    ...
    this.calculateOpacity = function () {
      ...
      let el = this;
      while (!isNullish(el)) {
        const opacityStyle = el.style('opacity', false, true); // no ancestors on style call
    ...
    this.setContext = function (ctx, fromMeasure) {
      if (!fromMeasure) { // causes stack overflow when measuring text with gradients
        ...
        if (this.style('stroke-dasharray').hasValue() && this.style('stroke-dasharray').value !== 'none') {
          ...
          const offset = this.style('stroke-dashoffset').toPixels();
      ...
      if (typeof ctx.font !== 'undefined') {
        ...
        const currentFontSize = this.style('font-size', false, false);
  ...
  svg.Element.svg = function (node) {
    ...
    this.setContext = function (ctx) {
      ...
      if (ctx.canvas.style && typeof ctx.font !== 'undefined' && typeof windowEnv.getComputedStyle !== 'undefined') {
        ...
        const fontSize = new svg.Property('fontSize', svg.Font.Parse(ctx.font).fontSize);
  ...
  svg.Element.path = function (node) {
    ...
    d = d.replace(/,/gum, ' '); // get rid of all commas
    // As the end of a match can also be the start of the next match, we need to run this replace twice.
    for (let i = 0; i < 2; i++) {
      d = d.replace(/([MmZzLlHhVvCcSsQqTtAa])([^\s])/gum, '$1 $2'); // suffix commands with spaces
    }
    d = d.replace(/([^\s])([MmZzLlHhVvCcSsQqTtAa])/gum, '$1 $2'); // prefix commands with spaces
    d = d.replace(/(\d)([+-])/gum, '$1 $2'); // separate digits on +- signs
    // Again, we need to run this twice to find all occurances
    for (let i = 0; i < 2; i++) {
      d = d.replace(/(\.\d*)(\.)/gum, '$1 $2'); // separate digits when they start with a comma
    }
    d = d.replace(/([Aa](\s+\d+)(?:\s+\d+)(?:\s+\d+))\s+([01])\s*([01])/gum, '$1 $3 $4 '); // shorthand elliptical arc path syntax
    ...
    this.PathParser = new function (d2) {
      ...
      this.isCommandOrEnd = function () {
        ...
        return !isNullish(this.tokens[this.i + 1].match(/^[A-Za-z]$/u));
      };
      ...
      while (!pp.isEnd()) {
        ...
        case 'a':
          while (!pp.isCommandOrEnd()) {
            ...
            // vector magnitude
            const m = function (v) { return Math.sqrt((v[0] ** 2) + (v[1] ** 2)); };
  ...
  svg.Element.GradientBase = function (node) {
    ...
    this.createGradient = function (ctx, element, parentOpacityProp) {
      ...
      if (this.attribute('gradientTransform').hasValue()) {
        ...
        const c = createCanvas(rootView.width, rootView.height);
  ...
  svg.Element.linearGradient = function (node) {
    ...
    this.getGradient = function (ctx, element) {
      const bb = this.gradientUnits() === 'objectBoundingBox' ? element.getBoundingBox(ctx) : null;
  ...
  svg.Element.radialGradient = function (node) {
    ...
    this.getGradient = function (ctx, element) {
      const bb = element.getBoundingBox(ctx);
      ...
      const fr = this.attribute('fr').toPixels();
  ...
  svg.Element.text = function (node) {
    ...
    this.getBoundingBox = function (ctx) {
      ...
      let bb = null;
      for (let i = 0; i < this.children.length; i++) {
        const childBB = this.getChildBoundingBox(ctx, this, this, i);
        if (isNullish(bb)) bb = childBB;
    ...
    this.getChildBoundingBox = function (ctx, textParent, parent, i) {
      ...
      const bb = child.getBoundingBox(ctx);

      for (let j = 0; j < child.children.length; j++) {
        const childBB = textParent.getChildBoundingBox(ctx, textParent, child, j);
  ...
  svg.Element.image = function (node) {
    ...
    if (href === '') { return; }
    const isSvg = href.match(/\.svg$/u);

    svg.Images.push(this);
    this.loaded = false;
    if (!isSvg) {
      this.img = nodeEnv ? new Canvas.Image() : doc.createElement('img');
      if (svg.opts.useCORS) { this.img.crossOrigin = 'Anonymous'; }
      const that = this;
      this.img.addEventListener('load', function () { that.loaded = true; });
  ...
  svg.ParseExternalUrl = function (url) {
    const [, , singleQuotes, doubleQuotes, noQuotes] = url.match(/url\(('([^']+)'|"([^"]+)"|([^'")]+))\)/u) || [];
        ...
        for (let j = 0; j < cssClasses.length; j++) {
          ...
          if (cssClass !== '') {
            ...
            if (cssClass === '@font-face' && !nodeEnv) {
              const fontFamily = props['font-family'].value.replace(/"/gu, '');
              const srcs = props.src.value.split(',');
              for (let s = 0; s < srcs.length; s++) {
                if (srcs[s].indexOf('format("svg")') > 0) {
                  const url = svg.ParseExternalUrl(srcs[s]);
                  if (url) {
                    const docum = svg.parseXml(svg.ajax(url));
                    const fonts = docum.getElementsByTagName('font');
  ...
  svg.Element.mask = function (node) {
    ...
    this.apply = function (ctx, element) {
      ...
      const mask = element.style('mask').value;
      element.style('mask').value = '';

      const cMask = createCanvas(x + width, y + height);
      ...
      const cm = new svg.Element.feColorMatrix({
        nodeType: 1,
        childNodes: [],
        attributes: [
      ...
      const c = createCanvas(x + width, y + height);
      const tempCtx = c.getContext('2d');
  ...
  svg.Element.clipPath = function (node) {
    ...
    this.apply = function (ctx) {
      const hasContext2D = (typeof CanvasRenderingContext2D !== 'undefined');
      const oldBeginPath = ctx.beginPath;
      const oldClosePath = ctx.closePath;
      if (hasContext2D) {
      ...
      for (let i = 0; i < this.children.length; i++) {
        ...
        if (typeof child.path !== 'undefined') {
          let transform = typeof child.elementTransform !== 'undefined' && child.elementTransform(); // handle <use />
  ...
  svg.Element.filter = function (node) {
    ...
    this.apply = function (ctx, element) {
      ...
      const c = createCanvas(width + 2 * px, height + 2 * py);
      const tempCtx = c.getContext('2d');
      svg.SetDefaults(tempCtx);
  ...
  svg.Element.feColorMatrix = function (node) {
    ...
    const includeOpacity = this.attribute('includeOpacity').hasValue();
    this.apply = function (ctx, x, y, width, height) {
      // assuming x==0 && y==0 for now
      const srcData = ctx.getImageData(0, 0, width, height);
      for (let y = 0; y < height; y++) { // eslint-disable-line no-shadow
        for (let x = 0; x < width; x++) { // eslint-disable-line no-shadow
          ...
          let nr = m(0, r) + m(1, g) + m(2, b) + m(3, a) + m(4, 1);
          let ng = m(5, r) + m(6, g) + m(7, b) + m(8, a) + m(9, 1);
          let nb = m(10, r) + m(11, g) + m(12, b) + m(13, a) + m(14, 1);
          let na = m(15, r) + m(16, g) + m(17, b) + m(18, a) + m(19, 1);
          if (includeOpacity) {
            nr = ng = nb = 0;
            na *= a / 255;
          }
  ...
  svg.CreateElement = function (node) {
    let className = node.nodeName.replace(/^[^:]+:/u, ''); // remove namespace
    className = className.replace(/-/gu, ''); // remove dashes
    let e = null;
  ...
  svg.loadXmlDoc = function (ctx, dom) {
    ...
    if (!svg.opts.ignoreMouse) {
      ctx.canvas.addEventListener('click', function (e) {
        const p = mapXY(new svg.Point(!isNullish(e) ? e.clientX : event.clientX, !isNullish(e) ? e.clientY : event.clientY)); // eslint-disable-line no-restricted-globals
      ...
      svg.intervalID = setInterval(function () {
        ...
        if (!svg.opts.ignoreAnimation) {
          for (let i = 0; i < svg.Animations.length; i++) {
            const needAnimationUpdate = svg.Animations[i].update(1000 / svg.FRAMERATE);
